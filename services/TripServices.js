...
export const TripServices = {
    ...
    // setInitialPrice: async (price) => {
    //     try { 
            
    //         //const url = //`https://fiuumber-api-users.herokuapp.com/api/trip-service/administrator`
    //         await axios.post(url,price, HEADERS);
    //         return true;

    //     }   
    //     catch (error) {
    //         console.log(`TripService setInitialPrice: ${error}`);
    //         if (error && error.response && error.response.status == 401) return null;
    //         throw error;
    //     }
    // },
